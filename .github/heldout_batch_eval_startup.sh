...
echo "=== arch heldout BATCH-eval boot $(date -u) — task=toysmoke3 ==="
...
SAFETY_NET_S=$(( HEAD_COUNT * 900 + 3600 ))
...
if ! git checkout "arch/toysmoke3"; then
  echo "ERROR: could not checkout base branch arch/toysmoke3. Keeping alive for debug."
  exec sleep infinity
fi
...
if [ -d "toysmoke3" ]; then cd "toysmoke3"; echo "cd into task subdir: $(pwd)"; fi

# ---- Prime the trusted base ref, ONCE, for every head's scorer-restore ----
git fetch origin "arch/toysmoke3" --depth=1 2>/dev/null || true
...
for i in "${!HEADS[@]}"; do
  ...
  if git fetch origin "$PR_SHA" && git checkout "$PR_SHA"; then
    ...
    for _tp in .arch; do
      if git checkout "origin/arch/toysmoke3" -- "$_tp" 2>/dev/null; then
