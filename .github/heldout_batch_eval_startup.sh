...
echo "=== arch heldout BATCH-eval boot $(date -u) — task=toysmoke ==="
...
if ! git checkout "arch/toysmoke"; then
  echo "ERROR: could not checkout base branch arch/toysmoke. Keeping alive for debug."
  exec sleep infinity
fi
...
if [ -d "toysmoke" ]; then cd "toysmoke"; echo "cd into task subdir: $(pwd)"; fi

# ---- Prime the trusted base ref, ONCE, for every head's scorer-restore ----
git fetch origin "arch/toysmoke" --depth=1 2>/dev/null || true
...
for i in "${!HEADS[@]}"; do
  ...
  if git fetch origin "$PR_SHA" && git checkout "$PR_SHA"; then
    ...
    for _tp in ['.arch']; do
      if git checkout "origin/arch/toysmoke" -- "$_tp" 2>/dev/null; then
