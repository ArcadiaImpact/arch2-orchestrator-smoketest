...
create `arch-toysmoke-20260819T215617Z-<asset_name>`, and upload it with `arch hf upload`.
...
`findings/toysmoke/problem.md` (the problem definition). They have not
...
1. Read this file, `findings/toysmoke/problem.md` (why this measurement makes sense
...
3. Branch off the task base. Use a hyphenated name, not a path nested under
   `arch/toysmoke` — git refuses a branch whose name extends an
   existing ref, and `arch/toysmoke` is the base branch you just
   cloned:

       git checkout -b arch-toysmoke-attempt-<short-slug>
...
7. Open the PR with `arch gh pr-submit` and a structured body:
   ...
   the `arch/toysmoke` base branch, prefixes the title with
   ...
   **Fallback** — only if `arch` is missing on this pod (its install is
   best-effort): use raw `gh` and record the number yourself.
       ...
       gh pr create --base arch/toysmoke --label arch/toysmoke-20260819T215617Z \
...
- Periodically `git fetch origin arch/toysmoke` and rebase your
...
- **Don't commit to the task branch `arch/toysmoke` directly.** It's
