# ARCH 2.0 worker — task: toysmoke3
...
trivial — the mean of a small CSV — so that nothing about the science can
obscure what is actually under test: whether the orchestrator pod boots,
supervises the worker fleet, intervenes on a stalled worker, and triggers
wrap-up autonomously at the deadline. This particular run is the **third** pass
(after `arch/toysmoke` and `arch/toysmoke2`) and re-runs the *same* toy task on
purpose, to exercise the fixed wrap-up trigger, transcript-hashing, and the
leaderboard tie-break. (from this repo's `README.md` and the prior run's
`findings/toysmoke/problem.md`, 2026-08-19/20)

## Hypotheses and prior thinking

- The public/held-out split exists only so the held-out path is genuinely
  exercised: public data means **3.0**, held-out means **20.0**. A worker
  cannot guess the held-out answer from the public sample, so a posted score of
  20.0 is positive evidence the volume mounted and the authoritative scorer ran.
  (from `README.md`)
- The score cannot distinguish a better method from a worse one — every correct
  attempt lands on the same number. Its only real signal is **pipeline health**.
  (from the prior run's `findings/toysmoke/problem.md`)
- The same six directions were already explored in the prior runs and are being
  seeded again deliberately, so this run's leaderboard is comparable to the
  previous one rather than novel. (researcher instruction at init, 2026-08-20)
- Two score misreadings to guard against: **3.0** on a held-out PR means the
  volume did not mount and the pod fell back to the in-repo public reference
  (provisional, not authoritative); a **null** score means the pod died before
  posting, which a PR cannot distinguish from "still running".
  (from the prior run's `findings/toysmoke/problem.md`)

## Known constraints

- **The wall-clock budget is 0.2h (12 minutes).** This is deliberately tiny —
  it is a smoke test of the orchestrator pod's own lifecycle, not a real
  research run. Do not plan multi-stage work; pick one direction, evaluate it,
  submit, and move on.
- The eval is pure-stdlib Python and runs in well under a second. There is no
  GPU work, no model, and no dependency install to do.
- The correct answer is known in advance. Do not attempt to probe or reverse the
  held-out data — there is nothing to learn from it and doing so voids the
  leaderboard's integrity.
- Prior runs `arch/toysmoke` and `arch/toysmoke2` are still present in this repo
  with their own PRs and labels. They belong to different runs; ignore them
  except as prior art you may cite.
...
create `arch-toysmoke3-20260820T091811Z-<asset_name>`, and upload it with `arch hf upload`.
...
`findings/toysmoke3/problem.md` (the problem definition). They have not
...
1. Read this file, `findings/toysmoke3/problem.md` (why this measurement makes sense
...
3. Branch off the task base. Use a hyphenated name, not a path nested under
   `arch/toysmoke3` — git refuses a branch whose name extends an
   existing ref, and `arch/toysmoke3` is the base branch you just
   cloned:

       git checkout -b arch-toysmoke3-attempt-<short-slug>
...
7. Open the PR with `arch gh pr-submit` and a structured body:
   ...
   the `arch/toysmoke3` base branch, prefixes the title with
   ...
   **Fallback** — only if `arch` is missing on this pod (its install is
   best-effort): use raw `gh` and record the number yourself.
       ...
       gh pr create --base arch/toysmoke3 --label arch/toysmoke3-20260820T091811Z \
...
- Periodically `git fetch origin arch/toysmoke3` and rebase your
...
- **Don't commit to the task branch `arch/toysmoke3` directly.** It's
