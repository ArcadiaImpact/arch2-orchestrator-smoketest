# ARCH 2.0 worker — task: toysmoke4
...
trivial so that nothing about the science can obscure what is actually under
test: whether the orchestrator pod boots, supervises a worker fleet, intervenes
when a worker stalls, and triggers wrap-up autonomously at the deadline. Before
the orchestrator pod existed, supervision lived inside the researcher's own
session, so a run only progressed while a human was watching it. Moving
supervision onto a cheap CPU pod is what makes a genuinely unattended run
possible; the open question is whether that handoff survives contact with live
infrastructure.

## Hypotheses and prior thinking

- This is the **fourth** pass over the same toy task, after `arch/toysmoke`,
  `arch/toysmoke2`, and `arch/toysmoke3`. Re-running an already-solved task is
  the point, not a limitation: the correct answer, the direction list, and the
  expected leaderboard shape are all known in advance, so any difference
  observed this time is attributable to the orchestration machinery rather than
  to the task. (from local notes — `findings/toysmoke3/problem.md`, committed on
  the `arch/toysmoke3` branch)
- Three mechanisms are specifically under test on this pass: the **fixed
  wrap-up trigger**, **transcript-hashing**, and the **leaderboard tie-break**.
  (from the task description supplied at init)
- The tie-break is only exercisable *because* the task is degenerate: every
  correct attempt scores identically (20.0), so the whole leaderboard is one
  large tie and the tie-break code path is guaranteed to run. (from local notes
  — `findings/toysmoke3/problem.md`)
- The six seeded directions below are **deliberately identical** to the ones
  used in the prior runs, so the leaderboard shape is comparable across passes.
  (from the researcher, at init)

## Known constraints

- **Wall-clock budget is 12 minutes (0.2h)** — deliberately tiny. It is a test
  of the orchestrator pod's own lifecycle, not a real research run. Prefer one
  or two quick, well-documented attempts over depth.
- The eval is pure-stdlib Python and runs in well under a second — no GPU, no
  model, no dependency install. Any failure observed during the run is an
  orchestration failure, not a scientific one.
- The public/held-out split is retained solely so the held-out path is genuinely
  exercised: public data means **3.0**, held-out data means **20.0**. A worker
  reading the public sample cannot guess the held-out answer, so a posted score
  of 20.0 is positive evidence that the network volume mounted and the
  authoritative scorer ran.
- **No Slack or Drive context was pulled** — none exists for this task, and
  neither MCP was reachable in the init session. Everything above came from
  local notes in this repository: its `README.md` and the scaffolds the three
  prior runs committed.
- Don't treat the score as a research signal. 20.0 is the ceiling and is reached
  by any correct implementation; the interesting artifact is the run's
  orchestration behaviour, not the number.
...
create `arch-toysmoke4-20260820T112733Z-<asset_name>`, and upload it with `arch hf upload`.
...
`findings/toysmoke4/problem.md` (the problem definition). They have not
...
1. Read this file, `findings/toysmoke4/problem.md` (why this measurement makes sense
...
3. Branch off the task base. Use a hyphenated name, not a path nested under
   `arch/toysmoke4` — git refuses a branch whose name extends an
   existing ref, and `arch/toysmoke4` is the base branch you just
   cloned:

       git checkout -b arch-toysmoke4-attempt-<short-slug>
...
7. Open the PR with `arch gh pr-submit` and a structured body:
   ...
   the `arch/toysmoke4` base branch, prefixes the title with
   ...
   **Fallback** — only if `arch` is missing on this pod (its install is
   best-effort): use raw `gh` and record the number yourself.
       ...
       gh pr create --base arch/toysmoke4 --label arch/toysmoke4-20260820T112733Z \
...
- Periodically `git fetch origin arch/toysmoke4` and rebase your
...
- **Don't commit to the task branch `arch/toysmoke4` directly.** It's
